function photoSlider(options) {
	...
	for (var i in defaults) {
		if (typeof options[i] === 'undefined') {
			...
			for (var deepDef in defaults[i]) {
				if (typeof options[i][deepDef] === 'undefined') {
					options[i][deepDef] = defaults[i][deepDef];
				}
			}
		}
	};
	...
	this.createDom();
	console.log(defaults.datajson);
	this.oUl = document.getElementById("pic-view");
	console.log(this.oUl);
	for(var j = 0; j < options.datajson.length; j++) {
		this.oUl.innerHTML += "<li><img /></li>";
		var $img = this.oUl.getElementsByTagName("img");
		$img[j].src = options.datajson[j];
	}

	this.init();
	this.bindEvent();
}
photoSlider.prototype.createDom = function(){
	var oUl = document.createElement("ul");
	oUl.id = "pic-view";
	oUl.className = "pic-view";
	this.wrap.appendChild(oUl);
}
